...
class OverlappingMemoryError(Exception):
    ...
    def __init__(
        ...
        locations_str = ", ".join(
            [
                (f"{start} to {end}" if start != end else f"{start}")
                for (start, end) in self.overlapping_locations
            ]
        )
        message = (
            f"Memory overlap detected. Overlapping positions are: {locations_str} "
...
class InvalidMemoryValueError(InvalidMemoryContentsError):
    def __init__(self, index: int, actual: int, expected: int):
        super().__init__(
            f"memory contains {actual}, but expected {expected}", index
        )
